...
def learning(config, result_path, run):
    ...
    for percept in range(NUM_STATES):
        ...
        INITIAL_DISTR.append([1-1e-2, 1e-2]) # in a state, [prob of not change, prob of change]
        
    print("Initial distr", INITIAL_DISTR)
    ...
    for e in range(EPISODES):
        print("Episode", e+1)
        # Initialize environment (new target pos, agent pos, reward) and agent state
        env.init_env()
        agent.phase = 0
        agent.duration = 0
        agent.reset_g()
        # print(agent.h_matrix)
        ...
        for time_index in range(1, (NUM_TAU*TIME_STEPS_TAU)+1):
            ...
            np.save(result_path+'memory_agent_'+str(run)+'_episode_'+str(e+1)+'.npy', agent.h_matrix)
            

learning(config, par.RESULTS_PATH, run=1)
